...
# PixArtGenerator moved to pipelines/pixart.py (see project-structure.md §2).
from pipelines.pixart import PixArtGenerator
...
def run_t2i(args) -> Dict:
    ...
    if args.method == "teacache":
        ...
        _bs = getattr(args, "speca_bucket_schedule", None)
        if _bs:
            speca_cache_dic.bucket_schedule = [int(x) for x in _bs.split(",")]
            if len(speca_cache_dic.bucket_schedule) != 3:
                raise ValueError("--speca-bucket-schedule must have 3 values")
