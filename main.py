...
Examples:
  ...
  python main.py --task t2i --dataset drawbench --n_prompts 1 \\
    ...
    parser.add_argument("--per-class-gamma", type=str, default=None,
                        help="Path to pre-computed per-class γ lookup table "
                             "(JSON from calibrate_per_class_gamma.py). "
                             "Zero-overhead O(1) dict lookup at inference. "
                             "Requires --method teacache.")
    # ---- Lever-reconstruction: TeaCache residual application mode ----
    parser.add_argument("--teacache-residual-mode", type=str, default="plain",
                        choices=["plain", "linear", "damped"],
                        help="Skip-step residual application: 'plain' = x+res "
                             "(vanilla); 'linear' = x+res+residual-drift "
                             "(validated: -53~66% latent MSE at equal skip); "
                             "'damped' = Hermite-scheduled drift.")
    parser.add_argument("--teacache-residual-decay", type=float, default=0.0,
                        help="Staleness-aware exponential decay λ on the cached "
                             "residual (0 = off).")
    # ---- Structure-aware design: SpecA time-bucket max_taylor schedule ----
    parser.add_argument("--speca-bucket-schedule", type=str, default=None,
                        help="Comma-separated max_taylor_steps per denoising "
                             "bucket, e.g. '8,4,2' (aggressive early, "
