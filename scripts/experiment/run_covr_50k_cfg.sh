...
# ============================================================================
# COVR Bandit 50k CFG=4.5 速度优化实验
# ============================================================================
# 三组实验分散到 3 张 GPU 并行：baseline / SpecA / COVR Bandit
# CFG (guidance_scale=4.5)，50k ImageNet，DiT-2-256
#
# 优化策略：
...
#
# 用法:
#   GPUS=0,1,2 bash scripts/run_covr_50k_cfg.sh
#
# 前台模式:
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/output/covr_50k_cfg/${RUN_ID}}"
...
SESSION_ID="covr-50k-cfg-${RUN_ID}"

# ---- Background launch logic ----
if [[ "${_COVR_50K_CFG_WORKER:-}" != "1" && "${FOREGROUND:-0}" != "1" ]]; then
  mkdir -p "${OUTPUT_ROOT}/logs"
  NOHUP_LOG="${OUTPUT_ROOT}/logs/_main.log"
  echo ""
  echo "  ┌─────────────────────────────────────────────────────────────┐"
  echo "  │  COVR 50k CFG=4.5 Experiment — launched in background        │"
  echo "  └─────────────────────────────────────────────────────────────┘"
  echo ""
  echo "  Config: 50k images, guidance_scale=4.5, DiT-2-256, 50 steps"
...
log "============================================="
log "COVR 50k CFG=4.5 Experiment"
log "============================================="
log "GPUs: ${GPUS}"
...
print()
print("=" * 80)
print(f"  COVR 50k CFG=4.5 Results (guidance_scale=4.5, DiT-2-256, 50 steps)")
print("=" * 80)
print()
...
summary = {"config": {"guidance_scale": 4.5, "n_prompts": 50000, "num_steps": 50,
                      "model": "dit", "note": "CFG=4.5, speed-optimized (no safety)"}}
for label, data in results.items():
