...
- **当前研究状态**（2026-08-20 更新）：COVR full-context 在线控制器主线 **STOP**（`covr_spec_research_conclusion.md`）；timestep feedback active 方向按质量 gate 关闭（2026-08-19，三环证据链闭合）；"Taylor term 范数前兆信号"离线 gate **负**（2026-08-20，`memory/project_taylor_term_norm_signal_offline_20260820.md`）。**P1 质量约束的在线预算校准：已关闭**（2026-08-21 Gate [P1-a] FAIL，5/5 offset 复现：conf 完美追踪 IS 但对 FID 轴致盲，FID-IS decoupling 下跟 IS 走；见 `memory/project_p1_conf_gate_fail_20260821.md`，规格 `.claude/covr_quality_constrained_budget_20260820.md` 已标 CLOSED）。**当前活跃方向 = mini-FID 哨兵**（P1 的唯一自然后继，换信号不换骨架；预注册规格 `.claude/covr_minifid_sentinel_20260821.md`，[P1'-a] 离线 gate 用同一批 static-run PNG，等 GPU 特征提取）。P1 离线 gate 工具链（五脚本 + `run_p1_offline_gates.sh`）保留为评估基础设施；预算梯控制器骨架已模块化为 `accelerators/conf_budget_controller.py`（信号无关，单测含与 gate 认证逻辑的逐 epoch 等价性），后继信号直接复用。VFL LoRA 训练信号源根因缺陷见 `memory/project_vfl_signal_source_flaw.md`。
