...
触发条件 (新):
  ...
  个 (layer, bucket) stratum 非空, 总事件数 ≥ ``buffer_ready_min_total_samples``,
  且 anchor 样本数 ≥ ``buffer_ready_min_anchors``。这避免了
...
class AsyncTrainingWorker:
    ...
    def _buffer_ready(self) -> bool:
        ...
        条件:
          1. 至少 ``buffer_ready_min_strata`` 个 stratum 非空
             (保证时序多样性 — 覆盖多个去噪阶段);
          2. 全 buffer 总事件数 ≥ ``buffer_ready_min_total_samples``
             (保证总体样本量 — 避免少量 event 反复采样严重过拟合);
          3. anchor 样本 ≥ ``buffer_ready_min_anchors``
        ...
        strata_ready = sum(1 for s in strata.values() if s.get("size", 0) > 0)
        total_ok = stats.get("total_samples", 0) >= cfg.buffer_ready_min_total_samples
        anchors_ok = stats.get("total_anchors", 0) >= cfg.buffer_ready_min_anchors

        ready = (strata_ready >= cfg.buffer_ready_min_strata) and total_ok and anchors_ok
        if ready:
            print(f"[VFL:AsyncTrainingWorker] buffer ready: "
                  f"{strata_ready}/{cfg.buffer_ready_min_strata} strata, "
                  f"{stats.get('total_samples', 0)} total events, "
    ...
    def _train_once(self) -> Optional[str]:
        ...
        # ---- 3. M 步梯度更新 ----
        losses: List[float] = []
        max_ev = min(cfg.max_events_per_step, len(usable_events))
        for _ in range(cfg.trainer_steps_per_trigger):
            # Per-step random subsample: caps memory + adds stochasticity.
            step_events = random.sample(usable_events, max_ev) if max_ev < len(usable_events) else usable_events
