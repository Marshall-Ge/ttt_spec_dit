...
class VerificationEvent:
    ...
    # SpecA block input hidden state — the hidden_states arriving at
    # check_layer before the block computes (i.e. Taylor-predicted path).
    # Used by L3 training for per-block replay instead of full SpecA forward.
    # None for TeaCache events or when --vfl-no-train.
    # ~590KB/event (DiT) instead of ~99MB for the full cache snapshot.
    block_input_hidden: Optional[torch.Tensor] = None
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "has_block_input_hidden": self.block_input_hidden is not None,
        }
...
def make_speca_event(
    ...
    block_input_hidden: the hidden_states arriving at check_layer before
    the block computes (Taylor-predicted path). Stored as cpu/half for
    per-block replay during L3 training — replaces the heavy SpecA cache
    snapshot (~590KB vs ~99MB per event).
    ...
    return VerificationEvent(
        ...
        block_input_hidden=(block_input_hidden.half().cpu()
